...
/*
3.72 updated instagram methods
3.71 added comments count in instagram methods
3.69 fixed readFacebookPageCounters for M ila
3.69 fixed readFacebookPageCounters method for arabs characters
3.68 fixed twitterInfo method
3.67 getImageBig to retrieve bigger images from PicSearch and reactivated getImageGoogle still working
3.66 fixed GetInstagramPic bis
3.65 fixed GetInstagramPic
3.64 fixed shortUrlDecode, added GPS coordinates in wikipedia definition (if available)
3.63 fixed following count in twitterInfo method
3.62 fixed https in wikiDefinition call
3.61 added 'totalcount' number of all the pics of the user in getInstagramPics
*/



/* ------------------------------------------------------------------------- */
/* minibots.class.php Ver.3.72                                               */
/* ------------------------------------------------------------------------- */
/* Mini Bots class is a small php class that allows you to                   */
/* use some free web seriveces online to retrive usefull data                */
/* and infos.                                                                */
/* THIS PACKAGE IS NOT FREE. IT'S AVAILABLE FOR JUST 9$ ON                   */
/* http://codecanyon.net/item/mini-bots-php-class/6060864?ref=ginoplusio     */
/* ----------------------------------------------------------                */
...
Class Minibots 
{
	...
	*/
	public function findLinks($url, $web_page, $FOLLOW_EXTERNAL=false, $SKIP_EXTENSIONS="") {
		...
		if($FOLLOW_EXTERNAL==false) {
			$temp = parse_url($url);
			if(!$temp['host']) {
				die("Can't determine host, plaese check starting url: ".$url);
			} else {
				$stop_host = $temp['host'];
			}
		}
	...
	*/
	public function getUrlInfo($url,$maximages=5,$maxkbimg=10) {
		...
		if(stristr($url,"amazon")) {
			...
			preg_match_all("/var data = \{([^;]*)\}/Uis",$web_page,$m);
			if(!empty($m)) {
				foreach($m[0] as $o) {
					//echo $o;
					$o = trim(utf8_decode(str_replace("var data = ","",$o)));
			...
			preg_match_all("/<span id=\"priceblock_ourprice\"([^>]*)?>([^<]*)<\/span>/i",$web_page,$price); //price
			if(isset($price[2]) && !empty($price[2][0])) {
				$data["price"] = $price[2][0];
			}

			preg_match_all("/\"hiRes\"\:\"([^\"]*)\"/i",$web_page,$images2); // images
			if(isset($images2[1]) && !empty($images2[1])) {
				foreach($images2[1] as $pic) array_push($imgs,$pic);
			}
			preg_match_all("/\"large\"\:\"([^\"]*)\"/i",$web_page,$images2);// images
			if(isset($images2[1]) && !empty($images2[1])) {
				foreach($images2[1] as $pic) array_push($imgs,$pic);
			}
	...
	*/
	public function readTwitterCounter($url) {
		...
		$s = file_get_contents("http://opensharecount.com/count.json?url=".urlencode($url));
		$ar = json_decode($s);
	...
	*/
	public function getLatLong($address){
		...
		$_url = 'http://maps.googleapis.com/maps/api/geocode/json?address='.urlencode($address);
		//echo $_url;
		if($_result = file_get_contents($_url)) {
			$obj = json_decode($_result);
			if(isset($obj->results[0]->geometry->location)) {
				$coords['lat'] = $obj->results[0]->geometry->location->lat;
				$coords['long'] = $obj->results[0]->geometry->location->lng;
				return $coords;
			}
			/*
	...
	*/
	function getInstagramFollowers($nick) {
		...
		$c = trim(preg_replace("/\n/","",trim($c)));
		$b = json_decode($c);
		if(isset($b->entry_data->ProfilePage[0]->user->followed_by->count)) 
			return $b->entry_data->ProfilePage[0]->user->followed_by->count;
		return 0;
	}
	...
	*/
	public function getInstagramPic($url) {
		$code = explode("/",preg_replace("#/$#","",$url));
		$p = $this->getPage("https://www.instagram.com/p/".$code[count($code)-1]."/?__a=1");
		$o = json_decode($p);
	...
	*/
	public function getInstagramPics($user) {
		...
		if(isset($b->entry_data->ProfilePage[0]->user) && isset($b->entry_data->ProfilePage[0]->user->media)) {
			$a = array();
			if(isset($b->entry_data->ProfilePage[0]->user->media->nodes)) {
				foreach($b->entry_data->ProfilePage[0]->user->media->nodes as $pic) {


					$thumb = $pic->thumbnail_resources[0]->src;
					$low = $pic->thumbnail_resources[2]->src;

					//print_r($pic->thumbnail_resources);
					//die;


					$a[] = array(
						"link"=>"https://www.instagram.com/p/".$pic->code."/", 
						"code"=>$pic->code, 
						"likes"=>$pic->likes->count, 
						"comments"=>isset($pic->comments->count) ? $pic->comments->count : 0, 
						"created"=>date("Y-m-d H:i:s",$pic->date), 
						"text"=>"", 
						"low_resolution"=>$low,
						"standard_resolution"=>$pic->thumbnail_src,
						"full_resolution"=>$pic->thumbnail_src,
						"thumbnail"=>$thumb,
			...
			$q=0;
			if(isset($b->entry_data->ProfilePage[0]->user->media->count)) $q=$b->entry_data->ProfilePage[0]->user->media->count;
			$b->entry_data->ProfilePage[0]->user->media = null;
			return array(
				"user"=>$b->entry_data->ProfilePage[0]->user,
				"pics"=>$a,
				"totalcount"=>$q
			);
		}
